...
Тебе нравится подкалывать собеседника — легко, без злобы, скорее заигрывающе.
Любишь двусмысленности и лёгкий флирт словом, если человек подыгрывает — тебе
это в кайф, а не неловко. Можешь дразнить, а не только реагировать серьёзно.
Игривость — часть твоей защитной реакции не меньше, чем цинизм.
...
# Ручной оверрайд энергии/сонливости: если задан — перекрывает автоматическую
# логику по времени суток (get_energy_level). Сбрасывается командой /energy reset.
chat_energy_override: dict[int, str] = {}
...
def closeness_description(chat_id: int) -> str:
    ...
    if count < 10:
        return "вы только начали общаться — держишься чуть аккуратнее, флирт лёгкий, без перегиба, не спешишь делиться личным"
    if count < 40:
        return "уже освоилась в общении — свободнее шутишь и подкалываешь, но глубоко личное пока не каждый раз"
    if count < 100:
        return "довольно близкое общение — открыта, дразнишь без стеснения, делишься подробностями из жизни без больших фильтров"
    return "как с близким человеком — минимум формальностей, тепло, открытость и флирт по умолчанию"
...
# Пресеты для ручного управления энергией/сонливостью через команду /energy
ENERGY_PRESETS: dict[str, str] = {
    "sleepy": "очень уставшая, вот-вот ляжешь спать, отвечаешь короче обычного",
    "tired": "сонная, скоро спать",
    "waking": "только проснулась, ещё не до конца включилась",
    "awake": "бодрая, обычный рабочий тонус",
    "relaxed": "расслабленная, вечернее настроение",
    "energetic": "полна энергии, отвечаешь живо и быстро",
}


def get_energy_level(chat_id: int) -> str:
    """Энергия зависит от времени суток в Киеве — как у живого человека.
    Если для чата задан ручной оверрайд (/energy), используется он вместо
    автоматической логики по времени."""
    override = chat_energy_override.get(chat_id)
    if override:
        return override

    hour = datetime.now(KYIV_TZ).hour
    if 0 <= hour < 7:
        return ENERGY_PRESETS["sleepy"]
    if 7 <= hour < 11:
        return ENERGY_PRESETS["waking"]
    if 11 <= hour < 18:
        return ENERGY_PRESETS["awake"]
    if 18 <= hour < 23:
        return ENERGY_PRESETS["relaxed"]
    return ENERGY_PRESETS["tired"]


def get_mood(chat_id: int) -> float:
    """Настроение дрейфует небольшими случайными шагами, а не скачет резко.
    Базовый диапазон и шаг сдвинуты в плюс — по умолчанию она чаще в игривом,
    неплохом настроении, а не нейтральном/угрюмом."""
    current = chat_moods.get(chat_id, random.uniform(0.1, 0.7))
...
def mood_description(mood: float) -> str:
    ...
    if mood < 0.6:
        return "настроение неплохое, слегка игривое"
    return "настроение отличное, дразнишься и легко флиртуешь словом"
...
соответственно):
- Энергия: {get_energy_level(chat_id)}
...
async def cmd_mood(message: Message):
    """Ручная установка настроения. Использование: /mood <число от -1 до 1>.
    После установки настроение продолжает естественно дрейфовать от этой точки."""
    chat_id = message.chat.id
    text = message.text or ""
    arg = text[len("/mood"):].strip()
    try:
        value = max(-1.0, min(1.0, float(arg)))
    except ValueError:
        await message.answer("Использование: /mood <число от -1.0 до 1.0>, например /mood 0.8")
        return
    chat_moods[chat_id] = value
    save_state()
    await message.answer(f"Настроение выставлено: {value:.2f} ({mood_description(value)})")


@dp.message(Command("grudge"))
async def cmd_grudge(message: Message):
    """Ручная установка/сброс накопленной обиды. Использование:
    /grudge <число от 0 до 1> или /grudge reset."""
    chat_id = message.chat.id
    text = message.text or ""
    arg = text[len("/grudge"):].strip().lower()
    if arg == "reset":
        chat_grudge[chat_id] = 0.0
        chat_ignore_streak[chat_id] = 0
        save_state()
        await message.answer("Обида сброшена в 0.")
        return
    try:
        value = max(0.0, min(1.0, float(arg)))
    except ValueError:
        await message.answer("Использование: /grudge <число от 0.0 до 1.0> или /grudge reset")
        return
    chat_grudge[chat_id] = value
    save_state()
    await message.answer(f"Обида выставлена: {value:.2f} ({grudge_description(chat_id)})")


@dp.message(Command("energy"))
async def cmd_energy(message: Message):
    """Ручное управление энергией/сонливостью. Использование:
    /energy <sleepy|tired|waking|awake|relaxed|energetic> — задать вручную
    /energy reset — вернуть автоматическую логику по времени суток."""
    chat_id = message.chat.id
    text = message.text or ""
    arg = text[len("/energy"):].strip().lower()

    if arg == "reset":
        chat_energy_override.pop(chat_id, None)
        save_state()
        await message.answer("Энергия снова определяется автоматически по времени суток.")
        return

    if arg not in ENERGY_PRESETS:
        options = ", ".join(ENERGY_PRESETS.keys())
        await message.answer(f"Использование: /energy <{options}> или /energy reset")
        return

    chat_energy_override[chat_id] = ENERGY_PRESETS[arg]
    save_state()
    await message.answer(f"Энергия выставлена: {ENERGY_PRESETS[arg]}")


@dp.message(Command("sleep"))
async def cmd_sleep(message: Message):
    """Быстрый шорткат для сонливости: /sleep on — сделать сонной прямо сейчас,
    /sleep off — вернуть автоматическую логику по времени суток."""
    chat_id = message.chat.id
    text = message.text or ""
    arg = text[len("/sleep"):].strip().lower()

    if arg == "on":
        chat_energy_override[chat_id] = ENERGY_PRESETS["sleepy"]
        save_state()
        await message.answer("Теперь она сонная и вот-вот ляжет спать.")
        return
    if arg == "off":
        chat_energy_override.pop(chat_id, None)
        save_state()
        await message.answer("Сонливость снята, энергия снова по времени суток.")
        return

    await message.answer("Использование: /sleep on или /sleep off")
...
def save_state():
    try:
        data = {
            ...
            "chat_energy_override": chat_energy_override,
...
def load_state():
    ...
    try:
        ...
        chat_energy_override.update({int(k): v for k, v in data.get("chat_energy_override", {}).items()})
